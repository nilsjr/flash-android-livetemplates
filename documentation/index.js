...
const liveTemplatesPath = path.join(process.cwd(), '..', 'livetemplates');
...
liveTemplateFiles.forEach((filename) => {
    ...
    if (liveTemplate && liveTemplate.templateSet && liveTemplate.templateSet.template) {
        ...
        let content = `# ${basename.replace(/Flash/g, '')}\n\n| Abbreviation | Description |\n| --- | --- |\n`;
...
const section = `Live templates\n-\n\n${newFiles.map(file => `* [${file.split('.md')[0].trim().replace(/Flash /g, '')}](documentation/${file.replace(/\s/g, '')})`).join('\n')}\n\nFeedback\n-`;
